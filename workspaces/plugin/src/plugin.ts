...
>(
  ...
  function getTemplateContent(filename: string) {
    ...
    try {
      ...
      if (verbose) {
        console.log(
          `[${pluginName}]: ${color.red(`Failed to load template: ${e}`)}`,
        );
      }
    }
  }
  ...
  return {
    ...
    configureServer(server) {
      ...
      middlewares.use(async (req, res, next) => {
        ...
        const fileNameReg = new RegExp(`${base}([^?]+)`);
        ...
        if (!fileName || !virtualPageMap[fileName]) {
          // This allows vite handling unmatched paths.
          return next();
        }
        ...
        if (!loadResult) {
          if (verbose) {
            console.log(
              `[${pluginName}]: ${color.red(`Failed to load url ${fileName}`)}`,
            );
          }
